...
class PropertyController extends Controller
{
    ...
    public function store(PropertyRequest $request)
    {
        ...
        // إرسال إشعار للأدمن فقط إذا كان العقار بحاجة لمراجعة (pending).
        if (($property->status ?? null) === 'pending') {
            try {
                $this->firebaseService->sendToAdmins(
                    'عقار جديد بانتظار المراجعة',
                    'يوجد عقار جديد من: ' . $request->user()->name . ' — ' . $property->title,
                    ['type' => 'property_created', 'id' => (string) $property->id]
                );
            } catch (\Exception $e) {
                // تجاهل خطأ الإشعار
            }
