...
use App\Http\Requests\SetSecurityQuestionsRequest;
...
class AuthController extends Controller
{
    ...
    public function setSecurityQuestions(SetSecurityQuestionsRequest $request): JsonResponse
    {
        try {
            $this->authService->updateSecurityAnswers(
                $request->user(),
                $request->only(['answer_1', 'answer_2'])
            );
            return $this->successResponse(null, 'تم حفظ إجابات الأسئلة الأمنية بنجاح');
        } catch (Throwable $e) {
            return $this->handleException($e, 'حفظ إجابات الأسئلة الأمنية');
        }
    }



    public function getQuestions(GetQuestionsRequest $request): JsonResponse
    {
        try {
            $questions = $this->authService->getSecurityQuestions($request->identifier);

            return $this->successResponse($questions, 'تمت العملية بنجاح');
        } catch (Throwable $e) {
            return $this->handleException($e, 'جلب الأسئلة الأمنية');
        }
    }

    public function verifyAnswers(VerifyAnswersRequest $request): JsonResponse
    {
        try {
            $result = $this->authService->verifySecurityAnswers(
                $request->identifier,
                $request->answer_1,
                $request->answer_2
            );

            return $this->successResponse($result, 'تم التحقق من الإجابات بنجاح');
        } catch (Throwable $e) {
            return $this->handleException($e, 'التحقق من الإجابات');
        }
    }
    ...
    private function handleException(Throwable $e, string $action): JsonResponse
    {
        ...
        switch ($e->getMessage()) {
            ...
            case 'wrong_answers':
                return $this->errorResponse('إجابات الأسئلة الأمنية غير صحيحة', 401);
            case 'firebase_user_not_allowed':
                return $this->errorResponse('Firebase هذا الإجراء غير متاح لمستخدمي', 403);
            case 'account_inactive':
                return $this->errorResponse('الحساب غير مفعل', 403);
            case 'account_banned':
                return $this->errorResponse('الحساب محظور. تواصل مع الدعم.', 403);
            case 'questions_not_set':
                return $this->errorResponse('لم يتم إﻋﺪاد الأسئلة الأمنية لهذا الحساب', 400);
            case 'invalid_or_expired_token':
                return $this->errorResponse('رمز إعادة التعيين غير صالح أو منتهي الصلاحية', 400);
            default:
                return $this->errorResponse("حدث خطأ أثناء $action", 500);
        }
    }

    public function resetPasswordWithToken(ResetPasswordWithTokenRequest $request): JsonResponse
    {
        try {
            ...
            return $this->handleException($e, 'إعادة تعيين كلمة المرور');
        }
    }
}
