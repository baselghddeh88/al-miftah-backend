...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        if (in_array($driver, ['mysql', 'mariadb'], true)) {
            DB::statement(
                "ALTER TABLE properties MODIFY COLUMN property_type ENUM('apartment','house','shop','land') NOT NULL"
            );
            return;
        }
        ...
        if ($driver === 'pgsql') {
            DB::statement("ALTER TABLE properties DROP CONSTRAINT IF EXISTS properties_property_type_check");
            DB::statement(
                "ALTER TABLE properties ADD CONSTRAINT properties_property_type_check CHECK (property_type IN ('apartment','house','shop','land'))"
            );
            return;
        }
    }
